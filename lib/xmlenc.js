...
function encryptKeyInfoWithScheme(symmetricKey, options, scheme, callback) {
  ...
  try {
    ...
    var params = {
      ...
      digestAlgorithm: options.digestAlgorithm ? `<e:EncryptionMethod Algorithm="${escapehtml(options.keyEncryptionAlgorithm)}"><DigestMethod Algorithm="${options.digestAlgorithm}" /></e:EncryptionMethod>` : `<e:EncryptionMethod Algorithm="${escapehtml(options.keyEncryptionAlgorithm)}" />`
    };
